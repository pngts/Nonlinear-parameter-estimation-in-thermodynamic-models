...
switch components
    case 2
        ...
        ff=@(du_fmin) 1/FIT_function_flexTP(du_fmin,2);
        %%%
        FITtol=0.01;
    ...
    case 3
        ...
        ff=@(du_fmin) 1/FIT_function_flexTP(du_fmin,3);
        FITtol=0.01;
        %%%
        minFITtarget=0.5;
        STOP_SENS=0.14;
    ...
    case 4
        if hybrid==1
            pop_size=200; %there is no need for large population in the hybrid algo.
        else
            pop_size=300;
        end
        number_of_generations = 10000;
        multi_pop_size=20;%poses fores polaplasios o arxikos plithismos
        ff=@(du_fmin) 1/FIT_function_flexTP(du_fmin,4);
        FITtol=0.01;
        %%%
        minFITtarget=0.5;
        STOP_SENS=0.25;
...
for i=1:multi_pop_size*pop_size
    ...
    gen_1_DuFITvalues(i,du_number+1) =FIT_function_flexTP(gen_1_DuFITvalues(i,:),components); % evaluate the fit function for each sol.
end
...
for i=pop_size/2+1 :pop_size %create the first score data
    ...
    gen_1_fitvalues(i) = FIT_function_flexTP(gen_1_Duvalues(:,i),components); % evaluate the fit function for each sol.
    
end
% protigennia=gen_1_Duvalues;
% save('outputB_stop_12x300.mat','protigennia') %% gia ta compare
...
while  i<number_of_generations
    ...
    for j=1:2:length(parents_loc)
        ...
        %% Creating kids
        kids(:,j)=ind_AB(:,1);
        kids_fitvalues(j) =FIT_function_flexTP(kids(:,j),components);
        
        kids(:,j+1) = ind_BA(:,1);
        kids_fitvalues(j+1) =FIT_function_flexTP(kids(:,j+1),components);
    ...
    for j=2:pop_size
        ...
        if GG==1
            gen_2_fitvalues(j)=FIT_function_flexTP(gen_2_Duvalues(:,j),components);
        end
    end
