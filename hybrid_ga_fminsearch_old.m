...
hybrid=0; % 1 gia na treksi to ivridio 0 mono o genetikos
stop_critirion=0;
switch components
    case 2
        pop_size = 50;
        ...
        STOP_SENS2=0.7; %protinw gia triadika sto 0.001 gia diadika 0.2 0.4
        clas_check=15;% kathe posa gen na ipologizi fminsearch
...
for i=1:multi_pop_size*pop_size
    ...
    fitness_tmp =FIT_function_flex(gen_1_DuFITvalues(i,:),components); % evaluate the fit function for each sol.
    gen_1_DuFITvalues(i,du_number+1) =  fitness_tmp;
end
...
for i=pop_size/2 :pop_size %create the first score data
    ...
    fitness_tmp = FIT_function_flex(gen_1_Duvalues(:,i),components); % evaluate the fit function for each sol.
...
while  i<number_of_generations
    ...
    gen_2_fitvalues=[];
    selected = [];
    ind_AB=zeros(du_number,1);ind_BA=zeros(du_number,1);
    ...
    for j=1:2:length(parents_loc)
        ...
        kids(:,j)=ind_AB(:,1);
        kids1(:)=kids(:,j);
        kids_fitvalues(j) =FIT_function_flex(kids1,components);
        
        kids(:,j+1) = ind_BA(:,1);
        kids2(:)=kids(:,j+1);
        kids_fitvalues(j+1) =FIT_function_flex(kids2,components);
...
disp_matrix2=['1/Q= ',num2str(FINALRES)];
disp(disp_matrix2)
% %% GRAPH
% x=1:10000;
