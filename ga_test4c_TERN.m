...
for i=pop_size/2 :pop_size %create the first score data
    ...
    fitness_tmp = Fit_function_TERN(gen_1_Duvalues(1,i),gen_1_Duvalues(2,i),gen_1_Duvalues(3,i),gen_1_Duvalues(4,i),gen_1_Duvalues(5,i),gen_1_Duvalues(6,i)); % evaluate the fit function for each sol.
...
for i=1:10000
    for ii=1:du_number
last_expdu(1,ii)= lastexplore(Dumax,Dumin,last(1,ii));
    end
last_expdu(1,7)=Fit_function_TERN(last_expdu(1,1),last_expdu(1,2),last_expdu(1,3),last_expdu(1,4),last_expdu(1,5),last_expdu(1,6));

if last_expdu(1,7)>last(1,7)
    for ii=1:du_number
All_generations_RESULTS(number_of_generations+1,ii)=last_expdu(1,ii);
    end
last(1,:)=last_expdu(1,:);
kk=kk+1
end
...
All_generations_RESULTS(number_of_generations+1,8) = toc;
