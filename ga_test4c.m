...
gen_1_Duvalues=[];
gen_1_DuFITvalues=[];
All_generations_RESULTS=zeros(number_of_generations+1,4);
fitness=0;
for i=1:multi_pop_size*pop_size
    ...
    fitness_tmp = Fit_function(gen_1_DuFITvalues(i,1),gen_1_DuFITvalues(i,2)); % evaluate the fit function for each sol.
...
for i=pop_size/2 :pop_size %create the first score data
    ...
    fitness_tmp = Fit_function(gen_1_Duvalues(1,i),gen_1_Duvalues(2,i)); % evaluate the fit function for each sol.
...
for i=1:number_of_generations
    ...
    for j=1:2:length(parents_loc)
        ...
        %% Creating kids
        kids(:,j)=ind_AB(:,1);
        kids_fitvalues(j) =Fit_function(kids(1,j),kids(2,j));

        kids(:,j+1) = ind_BA(:,1);
        kids_fitvalues(j+1) =Fit_function(kids(1,j+1),kids(2,j+1));      
       ...
       mutation_probability=mutation_probability^(1-i/number_of_generations);
        if ran_mut < mutation_probability
            if ran_mut < mutation_probability/3
                ...
                gen_2_fitvalues(j)=Fit_function(gen_2_Duvalues(1,j),gen_2_Duvalues(2,j));
            else
...
tic
last=zeros(1,4);
last(1,:)=All_generations_RESULTS(number_of_generations,:);
last_expdu=[0 0 0 0]; 
kk=0;
for i=1:10000
last_expdu(1,1)= lastexplore(Dumax,Dumin,last(1,1));
last_expdu(1,2)= lastexplore(Dumax,Dumin,last(1,2));
last_expdu(1,3)=Fit_function(last_expdu(1,1),last_expdu(1,2));

if last_expdu(1,3)>last(1,3)
All_generations_RESULTS(number_of_generations+1,1)=last_expdu(1,1);
All_generations_RESULTS(number_of_generations+1,2)=last_expdu(1,2);
All_generations_RESULTS(number_of_generations+1,3)=last_expdu(1,3);
last(1,:)=last_expdu(1,:);
kk=kk+1
end
...
All_generations_RESULTS(number_of_generations+1,4) = toc;
