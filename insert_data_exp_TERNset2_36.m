...
%% eisagogi dedomenwn 
[TK,PmmHg,X1,X2,Y1,Y2] = importfileTERN_excel('Acet-CHCl3-meth36.xlsx','Acet-CHCl3-meth2',6,155);
...
for i=1:length( X1 )
    for k=1:3
    gamma_exp(i,k)=Y(i,k).*PmmHg(i)./(X(i,k).*Ps(k));
    end
end
gamma_exp;
%% isagogi statherwn uniquac gia to dedomeno migma
R=1.9872;
r=[2.5735 2.87 1.4311];
q=[2.3360 2.41 1.4320]; q1=q;
gamma_cal=[];
%----------------------
